...
```
modules/
├── cloudfin-p2p.so      （P2P 网络发现+连接）
├── cloudfin-tor.so       （Tor 协议封装）
├── cloudfin-i2p.so       （I2P 协议封装）
├── cloudfin-crdt.so     （CRDT 同步引擎）
├── cloudfin-crypto.so   （端到端加密/密钥管理/数字签名）
└── cloudfin-storage.so   （本地数据存储）
```
