...
Cloudfin Core 需要一个支持 HTTP 和 WebSocket 的异步 Web 框架，并须与 Tokio 运行时深度集成。以下对三个主流候选框架进行对比分析。

| 维度 | Axum 0.7 | Actix-web 4 | Warp |
|------|----------|-------------|------|
| **运行时依赖** | 官方推荐 Tokio | 需额外配置 | 依赖 Tower/Ping Yin |
| **HTTP/WS 统一性** | 均通过 `axum::extract` 统一处理 | HTTP 与 WS 分离，API 不一致 | Filter 模式，WS 需特殊 filter |
...
**结论：** Axum 0.7 在异步原生设计、Tower 中间件生态、Tokio 深度集成三个维度均最优。Actix-web 功能强大但框架包袱较重；Warp 的 Filter 模式与项目整体设计语言契合度低。
...
> **注意：** 不使用 `Runtime::new()`（单线程），因为 Core 需要同时处理 HTTP 请求、WebSocket 消息和模块管理等多个并发任务源。
...
Core 的 HTTP 服务监听 `127.0.0.1:19001`，提供以下端点：

| 方法 | 路径 | 说明 | 认证 |
|------|------|------|------|
| `GET` | `/health` | 健康检查 | 无 |
| `GET` | `/api/v1/status` | 完整状态快照 | 无 |
| `POST` | `/api/v1/modules` | 加载模块 | 无（本地） |
| `DELETE` | `/api/v1/modules/{name}` | 卸载模块 | 无（本地） |
| `GET` | `/api/v1/config` | 获取当前配置 | 无 |
| `PATCH` | `/api/v1/config` | 热更新配置 | 无（本地） |
  ...
  "modules": [
    {
      ...
      "status": "running",
      "health": "healthy",
      "loaded_at": "2026-04-05T10:00:00Z"
    }
  ]
}
```

#### `PATCH /api/v1/config`（热更新）
...
```json
{
  ...
  "changes": {
    "log.level": "debug",
    "modules.trader-binance.enabled": true
  }
}
```
...
WebSocket 提供实时事件推送能力，连接地址为 `ws://127.0.0.1:19001/ws`。
...
```json
{
  "request_id": "req_abc123",       // 对应客户端请求，空字符串表示服务端主动推送
  "action": "status_snapshot",      // 消息类型
  "timestamp": "2026-04-05T14:00:00Z",
  "payload": { /* 消息内容 */ }
}
```

**客户端发送消息格式（Client → Server）：**
...
```json
{
  ...
  "action": "subscribe",
  "params": {
    "events": ["config_changed", "module_health"]
  }
}
```
...
| `status_snapshot` | S→C | 状态快照推送（服务端主动） |
...
| `MODULE_NOT_FOUND` | 404 | 模块不存在 | `DELETE` 未加载的模块 |
| `MODULE_ALREADY_LOADED` | 409 | 模块已加载 | 重复加载同名模块 |
| `MODULE_INIT_FAILED` | 500 | 模块初始化失败 | `.so` 入口返回错误 |
| `MODULE_START_FAILED` | 500 | 模块启动失败 | 模块内部错误 |
| `CONFIG_INVALID` | 400 | 配置项无效 | 热更新字段不存在 |
| `CONFIG_WRITE_FAILED` | 500 | 配置写入失败 | 磁盘权限错误 |
| `INTERNAL_ERROR` | 500 | Core 内部错误 | 未预期 panic |
...
1. UI（或任何调用方）发送 `PATCH /api/v1/config` 请求
...
5. 各订阅者（HTTP handler、模块）收到通知并应用新配置
...
1. Core 与 UI 之间的通信走 **IPC**（本地 HTTP）或 **SSH 隧道**（远程访问）
...
3. Core 启动后通过 `POST /api/v1/secrets` 注入（来自 UI 的加密传输）
...
{CoreRoot}/Cloudfin/
├── cloudfin.pid    # 进程 PID
├── cloudfin.sock   # Unix Domain Socket（本地 IPC，可选）
└── ...
```
